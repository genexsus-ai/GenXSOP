...
1. Add backend integration tests for `/forecasting/generate` diagnostics contract.
2. Persist diagnostics metadata for each run (optional audit table).
3. Add UI indicator for fallback reason per generated run.
4. Add drift monitoring over time (month-over-month metric degradation alerts).
